...
export function toApiError(
  ...
  if ((error as AxiosError)?.isAxiosError) {
    const ax = error as AxiosError<{ message?: string }>;
    const axStatus = ax.response?.status || status || 0;
    const axBody = ax.response?.data ?? body;
    const message =
      messageFromBody(axBody, '') ||
      ax.message ||
      `API ${axStatus || 'error'}`;
    return new ApiError(message, axStatus, axBody, path || ax.config?.url);
  }
  ...
  return new ApiError(messageFromBody(body, 'Unknown API error'), status, body ?? error, path);
}
