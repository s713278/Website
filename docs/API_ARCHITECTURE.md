...
| `orders.service.ts` | `placeOrder`, `listMyOrders`, `getMyOrder` | localStorage `md-customer-orders` | `POST /v1/orders/from-cart` (live checkout; `POST /v1/orders` is unimplemented), `GET /v1/users/{userId}/orders/history`, `GET /v1/users/{userId}/orders/{orderId}` |
