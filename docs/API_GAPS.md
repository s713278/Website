...
| `POST /v1/orders` is not implemented | Creating an order by any route but the cart | Returns `417 "createOrder from CreateOrderRequest not yet implemented"`. `POST /v1/orders/from-cart` is the only working creation route, and every order it creates arrives `order_status: SCHEDULED`, never `PENDING` — which is why the console's "New" bucket is `SCHEDULED`. Note the contract's own description of `POST /v1/orders` claims *"Order status set to PENDING by default"*; it cannot, because it does nothing. |
